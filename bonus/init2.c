...
/*   Updated: 2023/03/27 12:06:40 by fcherrie         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_textures(t_map *map)
{
	...
	if (tmp > 0)
		...
		return (-1);
	return (init_north(map, NULL) != 0 || init_south(map, NULL) != 0
		|| init_west(map, NULL) != 0 || init_east(map, NULL) != 0
		|| init_door(map, NULL) != 0 || init_do1(map, NULL) != 0
		|| init_do2(map, NULL) != 0 || init_do3(map, NULL) != 0
		|| init_do4(map, NULL) != 0 || init_do5(map, NULL) != 0
		|| init_do6(map, NULL) != 0 || init_do7(map, NULL) != 0
		|| init_do8(map, NULL) != 0);
}
